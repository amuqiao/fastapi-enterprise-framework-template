...
│   │   │       ├── __init__.py
│   │   │       └── connection.py
│   │   └── repositories/    # 仓储层实现目录，按数据库类型划分
│   │       ├── __init__.py
│   │       ├── base.py          # 仓储实现基类
│   │       ├── sqlite/          # SQLite仓储实现
│   │       │   ├── __init__.py
│   │       │   ├── user_repository.py    # SQLite用户仓储实现
│   │       │   └── order_repository.py   # SQLite订单仓储实现
│   │       └── mysql/           # MySQL仓储实现
│   │           ├── __init__.py
│   │           ├── user_repository.py    # MySQL用户仓储实现
│   │           └── order_repository.py   # MySQL订单仓储实现
...
- **domains/**：DDD领域模块目录，按业务域组织，包含业务模型、接口定义和业务逻辑
  - **domains/[domain]/repositories/**：各业务域的**仓储抽象接口**定义，体现业务需求，不包含具体实现

- **infrastructure/**：基础设施层目录，按技术功能组织，包含具体的技术实现
  - **infrastructure/database/**：数据库连接管理目录，包含数据库连接基类、管理器和具体数据库连接实现
  - **infrastructure/repositories/**：仓储层**具体实现**目录，按数据库类型划分，实现domains层定义的仓储接口
...
| 目录 | 职责 | 内容类型 | 命名说明 |
|------|------|----------|----------|
| `domains/[domain]/repositories/` | 定义业务数据访问契约 | 抽象接口 | 命名为`repositories`，表示这是领域模型的**仓储契约** |
| `infrastructure/repositories/` | 实现具体数据访问逻辑 | 具体类 | 命名为`repositories`，表示这是**仓储实现**，与抽象接口保持命名一致性便于理解 |

> **设计说明**：虽然两个目录名称相同，但它们处于不同的层级，承担不同的职责。这种设计遵循了DDD的依赖倒置原则，使业务逻辑与技术实现解耦。在实际项目中，开发者应根据上下文区分这两个目录的用途。

### 4. 仓储实现目录结构设计

#### 4.1 目录组织原则

`infrastructure/repositories/`目录采用**按数据库类型分组**的设计，而非按业务功能分组，这是基于以下考虑：

1. **技术相关性**：同一数据库类型的仓储实现具有相似的技术特性（如SQL方言、连接管理）
2. **便于维护**：修改数据库配置或迁移时，只需关注对应数据库类型的目录
3. **清晰的依赖关系**：每个仓储实现只依赖对应的数据库连接
4. **扩展性**：添加新数据库支持时，只需添加新的数据库类型目录

#### 4.2 目录结构详解

```
infrastructure/repositories/
├── base.py              # 所有仓储实现的基类，提取公共逻辑
├── sqlite/              # SQLite数据库的所有仓储实现
│   ├── user_repository.py    # SQLite用户仓储
│   └── order_repository.py   # SQLite订单仓储
└── mysql/               # MySQL数据库的所有仓储实现
    ├── user_repository.py    # MySQL用户仓储
    └── order_repository.py   # MySQL订单仓储
```

#### 4.3 设计优势

- **集中管理**：同一数据库的所有仓储实现集中管理
- **代码复用**：通过base.py提取公共逻辑，减少重复代码
- **清晰的层次结构**：数据库类型 → 具体业务仓储
- **符合基础设施层定位**：按技术功能组织，而非业务功能

#### 4.4 与业务功能组织的对比

| 组织方式 | 结构 | 适用场景 | 优势 | 劣势 |
|----------|------|----------|------|------|
| **按数据库类型** | `repositories/sqlite/user_repo.py` | 多数据库支持场景 | 技术相关代码集中，便于维护 | 同一业务的不同数据库实现分散 |
| **按业务功能** | `repositories/user/sqlite_repo.py` | 单数据库场景 | 同一业务的所有实现集中 | 跨业务的数据库配置分散 |

> **设计结论**：对于支持多数据库的系统，按数据库类型组织仓储实现是更优的选择，符合基础设施层按技术功能组织的原则。

### 4.5 业务逻辑与技术实现的清晰分离

#### 4.5.1 为什么不是杂糅？

虽然基础设施层的`repositories`目录按数据库类型组织，但**业务逻辑与技术实现仍然是清晰分离**的：

1. **业务逻辑在domains层**：`domains/user/`和`domains/order/`目录下分别包含各自的业务模型、服务和仓储接口，业务边界清晰
2. **技术实现在infrastructure层**：按数据库类型组织只是技术实现的组织方式，不影响业务逻辑的分离
3. **依赖关系单向**：infrastructure层依赖domains层的接口，domains层不依赖infrastructure层
4. **命名规范**：每个仓储实现类名和文件名都明确标识了业务领域（如`UserRepository`、`OrderRepository`）

#### 4.5.2 设计优化建议

如果希望在按数据库类型组织的同时，进一步增强业务区分度，可以考虑以下优化：

**优化方案1：在数据库类型目录下按业务领域子目录组织**

```
infrastructure/repositories/
├── sqlite/              # SQLite数据库
│   ├── user/            # 用户业务领域的SQLite实现
│   │   └── repository.py   # SQLite用户仓储
│   └── order/           # 订单业务领域的SQLite实现
│       └── repository.py   # SQLite订单仓储
└── mysql/               # MySQL数据库
    ├── user/            # 用户业务领域的MySQL实现
    │   └── repository.py   # MySQL用户仓储
    └── order/           # 订单业务领域的MySQL实现
        └── repository.py   # MySQL订单仓储
```

**优化方案2：使用清晰的命名规范**

确保所有仓储实现类和文件名都遵循统一的命名规范：
- 文件名：`[business_domain]_repository.py`（如`user_repository.py`）
- 类名：`[DatabaseType][BusinessDomain]Repository`（如`SQLiteUserRepository`）

#### 4.5.3 实际项目中的最佳实践

在实际项目中，选择哪种组织方式取决于：

1. **项目规模**：小型项目可以选择更简单的结构，大型项目需要更精细的组织
2. **多数据库需求**：强多数据库需求的项目适合按数据库类型组织
3. **团队习惯**：考虑团队成员的开发习惯和理解成本

#### 4.5.4 DDD原则的体现

无论选择哪种组织方式，都应确保：
- **依赖倒置**：高层模块（业务服务）只依赖抽象接口
- **关注点分离**：业务逻辑与技术实现分离
- **单一职责**：每个仓储类只负责一个业务领域的持久化
- **开闭原则**：添加新数据库支持时无需修改现有代码

> **核心设计思想**：基础设施层的组织方式是**技术实现细节**，不会影响domains层的业务逻辑分离。DDD的核心是**领域模型的清晰性**，而不是基础设施层的目录结构。
...
本设计采用**独立的依赖注入模块**来管理所有依赖注入，将依赖注入逻辑从各个业务模块中分离出来，集中在`dependencies/`目录下，实现了依赖注入的统一管理和配置，提高了代码的可维护性和可扩展性。
...
class DependencyContainer:
    ...
    def register(self, name: str, dependency: Callable) -> None:
        """注册依赖"""
...
from app.dependencies.database import db_deps

# 依赖注入函数

def get_sqlite_user_repository(db=db_deps.sqlite()):
    """SQLite用户仓储依赖注入"""
    return SQLiteUserRepository(db)


def get_mysql_user_repository(db=db_deps.mysql()):
    """MySQL用户仓储依赖注入"""
    return MySQLUserRepository(db)


def get_sqlite_order_repository(db=db_deps.sqlite()):
    """SQLite订单仓储依赖注入"""
    return SQLiteOrderRepository(db)


def get_mysql_order_repository(db=db_deps.mysql()):
    """MySQL订单仓储依赖注入"""
...
from app.domains.order.services.order_service import OrderService
from app.dependencies.repository import repo_deps

# 服务层依赖注入函数
def get_user_service():
    """用户服务依赖注入"""
    return UserService(repo_deps["user_repository"]())


def get_order_service():
    """订单服务依赖注入"""
    return OrderService(repo_deps["order_repository"]())
...
class ServiceDeps:
    ...
    @staticmethod
    def order_service():
        return Depends(get_order_service)
...
from fastapi import Depends
from app.config.settings import app_settings
from app.config.database import sqlite_config, mysql_config
...
# 导出基础组件
from app.dependencies.base import BaseDependency, DependencyContainer, dependency_container

# 导出各层依赖
from app.dependencies.database import db_deps, sqlite, mysql
from app.dependencies.repository import repo_deps
from app.dependencies.service import service_deps
from app.dependencies.config import config_deps
...
from app.domains.user.services.user_service import UserService
from app.dependencies import service_deps
