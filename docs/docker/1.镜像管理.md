...
flowchart TD
    subgraph 镜像分层结构
        A[基础镜像层<br/>base image] -->|添加| B[依赖层<br/>dependencies]
        B -->|添加| C[应用代码层<br/>application code]
        C -->|添加| D[配置层<br/>configuration]
        D -->|添加| E[运行时层<br/>runtime]
    end
...
flowchart TD
    subgraph 镜像构建流程
        A[编写 Dockerfile] -->|定义| B[构建指令]
        B -->|执行| C[docker build 命令]
        C -->|步骤1| D[解析 Dockerfile]
        D -->|步骤2| E[创建构建上下文]
        E -->|步骤3| F[拉取基础镜像]
        F -->|步骤4| G[执行 RUN 指令]
        G -->|步骤5| H[执行其他指令]
